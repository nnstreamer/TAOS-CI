...
##
# @brief nnstreamer app test to check plugin\
#
# This function is to test nnstreamer sample app
# 
function pr-nnstreamer-apptest() {
    echo "[DEBUG] Starting pr-nnstreamer-apptest function"
    ...
    cp nnstreamer_example_filter/nnstreamer_example_filter.py bin/
    ...
    # Download tensorflow-lite model file and labes.
    ...
    ## sholud be modified below.
    if [[ ${result} -ne 0 ]]; then
        echo "[DEBUG][FAILED] Oooops!!!!!! apptest is failed. Please check log files"
        echo ""
        #echo "======= Failed Apptest ======" >> ../report/build_log_${input_pr}_error.txt
    ...
    if [[ $check_result == "success" ]]; then
        ...
        #message=":octocat: **cibot**: $user_id, apptestcould not be completed. To find out the reasons, please go to ${CISERVER}/${PRJ_REPO_UPSTREAM}/ci/${dir_commit}/report/resource_check_${input_pr}_error.txt."
