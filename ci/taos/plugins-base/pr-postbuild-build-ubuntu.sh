...
function pr-postbuild-build-ubuntu-wait-queue(){
    ...
    cibot_report $TOKEN "pending" "${BOT_NAME}/pr-postbuild-build-ubuntu" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM_LOCAL}/ci/${dir_commit}/" "$GITHUB_WEBHOOK_API/statuses/$input_commit"
}
...
function pr-postbuild-build-ubuntu-run-queue(){
    ...
    if [[ $BUILD_MODE == 99 ]]; then
        ...
        cibot_report $TOKEN "success" "${BOT_NAME}/pr-postbuild-build-ubuntu" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM_LOCAL}/ci/${dir_commit}/" "$GITHUB_WEBHOOK_API/statuses/$input_commit"
        ...
        if [[ $check_result == "success" ]]; then
            ...
            cibot_report $TOKEN "failure" "${BOT_NAME}/pr-postbuild-build-ubuntu" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM_LOCAL}/ci/${dir_commit}/" "$GITHUB_WEBHOOK_API/statuses/$input_commit"
