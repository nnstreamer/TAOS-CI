...
# @brief [MODULE] TAOS/pr-postbuild-build-ubuntu-wait-queue
function pr-postbuild-build-ubuntu-wait-queue(){
    ...
    cibot_report $TOKEN "pending" "TAOS/pr-postbuild-build-ubuntu" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM}/ci/${dir_commit}/" "$GITHUB_WEBHOOK_API/statuses/$input_commit"
}

# @brief [MODULE] TAOS/pr-postbuild-build-ubuntu-ready-queue
function pr-postbuild-build-ubuntu-ready-queue(){
    message="Trigger: ready queue. The commit number is $input_commit."
    cibot_report $TOKEN "pending" "TAOS/pr-postbuild-build-ubuntu" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM}/ci/${dir_commit}/" "$GITHUB_WEBHOOK_API/statuses/$input_commit"
}

# @brief [MODULE] TAOS/pr-postbuild-build-ubuntu-run-queue
function pr-postbuild-build-ubuntu-run-queue(){
    message="Trigger: run queue. The commit number is $input_commit."
    cibot_report $TOKEN "pending" "TAOS/pr-postbuild-build-ubuntu" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM}/ci/${dir_commit}/" "$GITHUB_WEBHOOK_API/statuses/$input_commit"

    echo -e "########################################################################################"
    echo -e "[MODULE] TAOS/pr-postbuild-build-ubuntu: check build process for Ubuntu distribution"
    ...
    echo -e "[DEBUG] starting TAOS/pr-postbuild-build-ubuntu facility"
    ...
    if [[ $BUILD_MODE == 99 ]]; then
        ...
        cibot_report $TOKEN "success" "TAOS/pr-postbuild-build-ubuntu" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM}/ci/${dir_commit}/" "$GITHUB_WEBHOOK_API/statuses/$input_commit"
        ...
        echo -e "[DEBUG] pdebuild procedure is skipped - it is ready to review! :shipit: Note that CI bot has two sub-bots such as TAOS/pr-postbuild-all and TAOS/pr-prebuild-all."
    else
        echo -e "BUILD_MODE != 99"
        ...
        if [[ $check_result == "success" ]]; then
            ...
            cibot_report $TOKEN "failure" "TAOS/pr-postbuild-build-ubuntu" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM}/ci/${dir_commit}/" "$GITHUB_WEBHOOK_API/statuses/$input_commit"
