...
# The execution result is reported with "TAOS/pr-prebuild-doxygen-tag" context into status section
...
##
# @brief [MODULE] TAOS/pr-prebuild-doxygen-tag
function pr-prebuild-doxygen-tag(){
    echo "########################################################################################"
    echo "[MODULE] TAOS/pr-prebuild-doxygen-tag: Check if source code includes required doxygen tags for doxygen documentation."
    ...
    if [[ $check_result == "success" ]]; then
        ...
        cibot_report $TOKEN "success" "TAOS/pr-prebuild-doxygen-tag" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM}/ci/${dir_commit}/" "${GITHUB_WEBHOOK_API}/statuses/$input_commit"
        ...
        cibot_report $TOKEN "failure" "TAOS/pr-prebuild-doxygen-tag" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM}/ci/${dir_commit}/" "${GITHUB_WEBHOOK_API}/statuses/$input_commit"
