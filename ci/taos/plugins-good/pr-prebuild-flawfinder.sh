...
function pr-prebuild-flawfinder(){
    echo "########################################################################################"
    echo "[MODULE] ${BOT_NAME}/pr-prebuild-flawfinder: Check security problems in C/C++ source codes with flawfinder"
    pwd
    ...
    if [[ $check_result == "success" ]]; then
        ...
        cibot_report $TOKEN "success" "${BOT_NAME}/pr-prebuild-flawfinder" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM_LOCAL}/ci/${dir_commit}/" "${GITHUB_WEBHOOK_API}/statuses/$input_commit"
        ...
        cibot_report $TOKEN "failure" "${BOT_NAME}/pr-prebuild-flawfinder" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM_LOCAL}/ci/${dir_commit}/" "${GITHUB_WEBHOOK_API}/statuses/$input_commit"
