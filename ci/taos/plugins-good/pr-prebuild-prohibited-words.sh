...
# @brief [MODULE] TAOS/pr-prebuild-prohibited-words
function pr-prebuild-prohibited-words(){
    echo -e "########################################################################################"
    echo -e "[MODULE] TAOS/pr-prebuild-prohibited-words: Check if there are prohibited words in the text files."
    pwd
    ...
    if [[ $check_result == "success" ]]; then
        ...
        cibot_report $TOKEN "success" "TAOS/pr-prebuild-prohibited-words" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM}/ci/${dir_commit}/" "${GITHUB_WEBHOOK_API}/statuses/$input_commit"
        ...
        cibot_report $TOKEN "failure" "TAOS/pr-prebuild-prohibited-words" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM}/ci/${dir_commit}/" "${GITHUB_WEBHOOK_API}/statuses/$input_commit"
