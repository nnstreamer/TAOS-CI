...
##
# @file     pr-prebuild-rpm-spec.sh
# @brief    Check the spec file (packaging/*.spec) with rpmlint command
# @see      https://github.com/nnsuite/TAOS-CI
# @author   Geunsik Lim <geunsik.lim@samsung.com>
#

# @brief [MODULE] TAOS/pr-prebuild-rpm-spec
function pr-prebuild-rpm-spec(){
    echo "########################################################################################"
    echo "[MODULE] TAOS/pr-prebuild-rpm-spec: Check if a spec file is correctly written using rpmlint"
    spec_modified="false"
    ...
    if [[ spec_modified == "true" ]]; then
        ...
        if [[ $check_result == "success" ]]; then
            ...
            cibot_report $TOKEN "success" "TAOS/pr-prebuild-rpm-spec" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM}/ci/${dir_commit}/" "${GITHUB_WEBHOOK_API}/statuses/$input_commit"
        else
            echo "[DEBUG] Failed. the rpm spec checker."
            message="Oooops. The rpm spec checker is failed."
            cibot_report $TOKEN "failure" "TAOS/pr-prebuild-rpm-spec" "$message" "${CISERVER}${PRJ_REPO_UPSTREAM}/ci/${dir_commit}/" "${GITHUB_WEBHOOK_API}/statuses/$input_commit"
        fi
    else
